...
#                 debit += line.credit
        ...
        if (partner.credit - partner.debit + self.amount_total) > partner.credit_limit:
            if not partner.over_credit:
                msg = 'Can not confirm Sale Order,Total mature due Amount ' \
                      '%s as on %s !\nCheck Partner Accounts or Credit ' \
                      'Limits !' % (partner.credit - partner.debit, today_dt)
                raise UserError(_('Credit Over Limits !\n' + msg))
            partner.write({'credit_limit': partner.credit - partner.debit + self.amount_total})
        return True
